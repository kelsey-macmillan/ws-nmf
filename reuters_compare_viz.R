...
# Data import
df_lda = read.csv('results/reuters_lda.csv') %>%
  select(2:4) %>%
  mutate(method='LDA')

df_nmf = read.csv('results/reuters_nmf.csv') %>%
  select(2:4) %>%
  mutate(method='NMF')

df_tsnmf = read.csv('results/reuters_wsnmf_large.csv') %>%
  select(2:6)

df_tsnmf_05 <- df_tsnmf %>%
  filter(supervision==0.05) %>%
  select(one_of(c('avg_similarity','n_topics_resolved','rep'))) %>%
  mutate(method='TS-NMF 0.05')
...
df <- rbind(df_lda,df_nmf,df_tsnmf_05, df_tsnmf_20,df_tsnmf_50,df_tsnmf_80) %>%
...
ggsave('results/comparemodels.png', p1)
